...
- **データ集計の統一フォーマット（ユーザー指定、2026-08）**: ドリル表示に限らず、
  **あらゆる検証・計測（自己プレー、A/B、ハーネス実行）の結果報告は必ず**次の3行で出す:
  ①`通算成績 ハンド数 N / FL突入率 x% / ファウル率 y%`
  ②`素点平均 a / FL価値込み平均 b / 二重計上FL価値込み平均 c`（b は補正済みテーブル、
  c は未補正 = 参考ソルバー会計。`UNCORRECTED_FL_VALUES(_JOKER)`）
  ③`FL内訳: QQ:_% KK:_% AA:_% tri:_%`
  補足指標（対戦μ、リステイ率等）は3行の後に追加してよい。ハーネスの出力もこの形式に揃える。
...
- **用語の統一**: エクイティ/期待値の表示は EQ / EV。プレイヤー名は Hero / Villain。
  **ストリート番号（ユーザー指定、2026-08）**: 最初の5枚配置 = 第1ストリート、以降の
  「3枚引いて2枚置き1枚捨て」の4回 = 第2〜第5ストリート。（旧記法の「初手+第1〜4ストリート」
  とは1ずれるので注意。盤面枚数との対応: 第1St後=5枚, 第2St後=7枚, 第3St後=9枚,
  第4St後=11枚, 第5St後=13枚完成。）
...
- ジョーカー（54枚デッキ）: `rank: 0` のワイルド2枚（`cards.ts` の `JOKER_CARDS`、ID 52/53、コード "X1"/"X2"）。
  **ルームルール（ユーザー裏取り済み・2026-08 確定）: 「ファウルしない置換の中で最強のカード」になる**
  （EvenBet/PPPoker/Phenom の各ジョーカーOFCと同方式）。5オブアカインドは無し。
  解決は下から貪欲: ボトムは無制約最大 → ミドルはボトム以下で最大 → トップはミドル以下で最大。
  どう置換しても順序を満たせない場合のみファウル（`score.ts` の `evaluateArrangement`）。
  段単位の関数（`evaluate3/5`、fastEval の key）は従来通り段内最大化で、盤面レベルの
  ファウル回避は evaluateArrangement / 各ソルバーの盤面評価側で行う。参照実装は置換総当たり、
...
- **候補評価の未来モデル** (`evaluateBoard` の `futureModel`): 既定 `'combined'` = 逐次プレイ
  ...
  `'rollout'`（旧・解析用）= 逐次最適プレイのロールアウト。実測で全予算帯 policy に劣後し
  解析からは撤去（コードは維持、2026-09）。
  `'oneshot'`（現・解析の初手）= 「N*枚を一気に配って一括最適配置」サロゲート。
  N* = `ONESHOT_N_DEFAULT` = 8.79 はセル完全解 V1=15.1413 への較正値（`cellOneShot.test.ts`、
  2万サンプル）。逐次プレー（12枚見て8枚置く）と価値等価。初手は**粗選別なしの全候補直当て**
  （勉強用・品質最大のユーザー合意設計）。解析のストリートは policy512 +
  `endgameNeed4`（第3St 2段厳密、フラッシュ不能盤面はスケルトン共有）+
  `endgameExact`（第4St全列挙）。第5Stの決定論厳密は全精度で常時オン。
  **セル完全解**（M[KK]B[653]: FL 36.68% / ファウル 25.27% / 素点 5.960、1000ハンド裏取り済み）は
  `data/cell-KK653/` と `cellSolve*.test.ts` 参照。新規モデルの較正はこの厳密値を物差しにする。
