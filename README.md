...
- 推奨スコアの重みは FL 価値・ファウルコストの実測値ベース（`DEFAULT_FL_VALUES` = 14枚:14.5 /
  15枚:18.7 / 16枚:26.2 / 17枚:34.9 点、`DEFAULT_FOUL_WEIGHT` = 9.0 点）。ルーム固有のレートに
  合わせた再測定・設定化は今後の課題。ジョーカー入り（54枚）用の FL 価値テーブルも未導出
  （現状は52枚実測値を流用）。
